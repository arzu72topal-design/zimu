...
const DEFAULT_MEMORY_FOLDERS = [
  { id:"tatiller", name:"Tatiller", color:"#1D9E75", icon:"T" },
  { id:"aile", name:"Aile", color:"#185FA5", icon:"A" },
  { id:"arkadaslar", name:"Arkadaşlar", color:"#534AB7", icon:"Ar" },
];
const MOOD_LIST = [
  { id:"happy", label:"Mutlu", color:"#1D9E75", symbol:"☺" },
  { id:"love", label:"Aşk", color:"#ec4899", symbol:"♥" },
  { id:"excited", label:"Heyecanlı", color:"#BA7517", symbol:"★" },
  { id:"peaceful", label:"Huzurlu", color:"#06b6d4", symbol:"◉" },
  { id:"sad", label:"Üzgün", color:"#534AB7", symbol:"◎" },
  { id:"angry", label:"Kızgın", color:"#D85A30", symbol:"▲" },
  { id:"thoughtful", label:"Düşünceli", color:"#8b5cf6", symbol:"◆" },
  { id:"funny", label:"Komik", color:"#D85A30", symbol:"✦" },
];
...
export default function MemoriesRoom({ data, update, onBack }) {
  ...
  const [folderForm, setFolderForm] = useState({ name:"", color:"#1D9E75", icon:"N" });
  ...
  const addFolder = () => {
    ...
    setShowFolderModal(false); setFolderForm({ name:"", color:"#1D9E75", icon:"N" });
  };
      ...
      <div className="room-enter">
        <StickyHeader>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <button onClick={()=>editMemory(activeItem)} style={{background:"rgba(24,95,165,0.08)",border:"none",color:"#185FA5",width:34,height:34,borderRadius:10,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✎</button>
            <button onClick={()=>deleteMemory(activeItem.id)} style={{background:"rgba(239,68,68,0.1)",border:"none",color:"#D85A30",width:34,height:34,borderRadius:10,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          </div>
        </StickyHeader>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12,flexWrap:"wrap"}}>
          ...
          <span style={{fontSize:12,color:"#8B8578"}}>{activeItem.date}</span>
          {fi&&<span style={{fontSize:11,background:`${fi.color}20`,color:fi.color,padding:"2px 10px",borderRadius:8}}>{fi.icon} {fi.name}</span>}
        </div>

        {/* Text */}
        {activeItem.text&&(
          <div style={{fontSize:15,lineHeight:1.7,color:"#2C2A26",marginBottom:16,whiteSpace:"pre-wrap"}}>{activeItem.text}</div>
        )}

        {/* Tags */}
        {activeItem.tags?.length>0&&(
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {activeItem.tags.map(t=><span key={t} style={{background:"rgba(34,197,94,0.15)",color:"#1D9E75",padding:"3px 10px",borderRadius:8,fontSize:11}}>#{t}</span>)}
          </div>
        )}
      </div>
    );
      ...
      <div className="room-enter">
        <StickyHeader>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            ...
            <span style={{fontSize:11,color:"#8B8578"}}>{filteredItems.length} {T("memoryCount")}</span>
          </div>
          <input value={search} onChange={e=>setSearch(e.target.value)}
            placeholder={T("searchMemories")} style={{
            width:"100%",padding:"8px 12px",borderRadius:10,
            background:"rgba(0,0,0,0.06)",border:"1px solid rgba(0,0,0,0.06)",
            color:"#2C2A26",fontSize:13,outline:"none",boxSizing:"border-box",
          }}/>
        </StickyHeader>
        ...
        {filteredItems.length===0?(
          <div style={{textAlign:"center",padding:"40px 20px"}}>
            <div style={{marginBottom:12}}>
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none"><rect x="2" y="6" width="20" height="14" rx="3" stroke="#1D9E75" strokeWidth="1.5" fill="rgba(34,197,94,0.1)"/><circle cx="12" cy="13" r="4" stroke="#1D9E75" strokeWidth="1.5"/><path d="M8 6V5a2 2 0 012-2h4a2 2 0 012 2v1" stroke="#1D9E75" strokeWidth="1.5"/></svg>
            </div>
            <div style={{fontSize:14,fontWeight:600,color:"#8B8578",marginBottom:6}}>{T("noMemories")}</div>
            <div style={{fontSize:12,color:"#8B8578"}}>{T("addFirstMemory")}</div>
          </div>
        ):(
              ...
              <div key={item.id} className="touch-card" onClick={()=>openDetail(item)} style={{
                ...
                {item.photos?.[0]?(
                  ...
                  <div style={{width:80,height:80,flexShrink:0,background:"rgba(0,0,0,0.03)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28}}>
                    {item.mood||<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="#8B8578" strokeWidth="1.5"/><path d="M14 2v6h6" stroke="#8B8578" strokeWidth="1.5"/><line x1="8" y1="13" x2="16" y2="13" stroke="#8B8578" strokeWidth="1" opacity=".5"/><line x1="8" y1="17" x2="12" y2="17" stroke="#8B8578" strokeWidth="1" opacity=".5"/></svg>}
                  </div>
                )}
                <div style={{flex:1,padding:"10px 12px 10px 0",minWidth:0}}>
                  ...
                  {item.text&&<div style={{fontSize:12,color:"#8B8578",marginBottom:4,
                    display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden",lineHeight:1.4}}>{item.text}</div>}
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{fontSize:10,color:"#8B8578"}}>{item.date}</span>
                    {item.photos?.length>1&&<span style={{fontSize:10,color:"#8B8578"}}>+{item.photos.length} foto</span>}
                  </div>
                </div>
              </div>
            );
        ...
        <FAB onClick={()=>{setEditingId(null);setForm({title:"",text:"",date:today(),mood:"",folder:activeFolder||"",tags:"",photos:[]});setShowAdd(true);}} color="#1D9E75"/>
        ...
        <Modal open={showAdd} onClose={()=>{setShowAdd(false);setEditingId(null);}} title={editingId?T("editMemory"):T("newMemory")}>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:12,color:"#8B8578",marginBottom:6}}>{T("memoryMood")}</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {MOOD_LIST.map(m=>(
                <button key={m.symbol} onClick={()=>setForm({...form,mood:form.mood===m.symbol?"":m.symbol})} style={{
                  ...
                  color:form.mood===m.symbol?m.color:"#8B8578",
          ...
          {/* Folder selector */}
          <div style={{marginBottom:10}}>
            <div style={{fontSize:12,color:"#8B8578",marginBottom:6}}>{T("memoryFolder")}</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          {/* Photos */}
          <div style={{marginBottom:10}}>
            <div style={{fontSize:12,color:"#8B8578",marginBottom:6}}>{T("memoryPhotos")}</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
              {form.photos.map((p,i)=>(
                <div key={i} style={{position:"relative",width:72,height:72}}>
                  ...
                  <button onClick={()=>removePhoto(i)} style={{
                    ...
                    background:"#D85A30",color:"#fff",border:"none",fontSize:10,cursor:"pointer",
              ...
              {form.photos.length<8&&(
                <button onClick={()=>fileRef.current?.click()} style={{
                  ...
                  background:"rgba(0,0,0,0.03)",color:"#8B8578",cursor:"pointer",
                  display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,fontSize:10,
                }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="14" rx="2" stroke="#8B8578" strokeWidth="1.5"/><circle cx="8.5" cy="10.5" r="2" stroke="#8B8578" strokeWidth="1.5"/><path d="M3 16l4-4 3 3 4-5 7 6" stroke="#8B8578" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  {T("addPhoto")}
                </button>
              )}
            </div>
    ...
    <div className="room-enter">
      <StickyHeader>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:20}}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="2" y="6" width="20" height="14" rx="3" stroke="#1D9E75" strokeWidth="1.5"/><circle cx="12" cy="13" r="4" stroke="#1D9E75" strokeWidth="1.5"/><path d="M8 6V5a2 2 0 012-2h4a2 2 0 012 2v1" stroke="#1D9E75" strokeWidth="1.5"/></svg>
          </span>
          <h3 style={{margin:0,fontSize:19,fontWeight:800,flex:1}}>{T("memories")}</h3>
          <span style={{fontSize:11,color:"#8B8578"}}>{items.length} {T("memoryCount")}</span>
        </div>
      </StickyHeader>
      ...
      <div className="touch-card" onClick={openAll} style={{
        ...
        <div style={{width:44,height:44,borderRadius:12,background:"rgba(34,197,94,0.15)",
          display:"flex",alignItems:"center",justifyContent:"center"}}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3 7v12a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" stroke="#1D9E75" strokeWidth="1.5" fill="rgba(34,197,94,0.15)"/></svg>
          </div>
        <div style={{flex:1}}>
          <div style={{fontSize:15,fontWeight:700}}>{T("allMemories")}</div>
          <div style={{fontSize:12,color:"#8B8578"}}>{items.length} {T("memoryCount")}</div>
        </div>
        <span style={{color:"#8B8578",fontSize:14}}>▶</span>
      </div>

      {/* Folder grid */}
      <div style={{fontSize:12,fontWeight:700,color:"#8B8578",textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>{T("folders")}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        ...
        <div className="touch-card" onClick={()=>setShowFolderModal(true)} style={{
          ...
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="#8B8578" strokeWidth="2" strokeLinecap="round"/></svg>
          <div style={{fontSize:12,color:"#8B8578"}}>{T("newFolder")}</div>
        </div>
      </div>
      ...
      {items.length>0&&(
        <>
          <div style={{fontSize:12,fontWeight:700,color:"#8B8578",textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>{T("timeline")}</div>
            ...
            <div key={item.id} className="touch-card" onClick={()=>{setActiveFolder(null);setView("list");setTimeout(()=>openDetail(item),50);}} style={{
              ...
              {item.photos?.[0]?(
                ...
                <div style={{width:40,height:40,borderRadius:8,background:"rgba(0,0,0,0.06)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  {item.mood ? <span style={{fontSize:16}}>{item.mood}</span> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="#8B8578" strokeWidth="1.5"/><path d="M14 2v6h6" stroke="#8B8578" strokeWidth="1.5"/></svg>}
                </div>
              )}
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.title}</div>
                <div style={{fontSize:11,color:"#8B8578"}}>{item.date}</div>
              </div>
            </div>
          ))}
        </>
      ...
      {/* Add memory FAB */}
      <FAB onClick={()=>{setEditingId(null);setForm({title:"",text:"",date:today(),mood:"",folder:"",tags:"",photos:[]});setShowAdd(true);}} color="#1D9E75"/>
      ...
      <Modal open={showFolderModal} onClose={()=>setShowFolderModal(false)} title={T("newFolder")}>
        ...
        <div style={{fontSize:12,color:"#8B8578",marginBottom:6}}>Renk</div>
        ...
        <div style={{fontSize:12,color:"#8B8578",marginBottom:6}}>İkon</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
          {FOLDER_ICONS.map(ic=>(
            <button key={ic} onClick={()=>setFolderForm({...folderForm,icon:ic})} style={{
              width:36,height:36,borderRadius:10,border:folderForm.icon===ic?"2px solid #1D9E75":"2px solid rgba(0,0,0,0.08)",
        ...
        <div style={{marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
            {form.photos.map((p,i)=>(
              <div key={i} style={{position:"relative",width:72,height:72}}>
                <img src={p} alt="" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:10}}/>
                <button onClick={()=>removePhoto(i)} style={{position:"absolute",top:-6,right:-6,width:20,height:20,borderRadius:"50%",background:"#D85A30",color:"#fff",border:"none",fontSize:10,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
              </div>
            ))}
            {form.photos.length<8&&(
              <button onClick={()=>fileRef.current?.click()} style={{width:72,height:72,borderRadius:10,border:"2px dashed rgba(0,0,0,0.1)",background:"rgba(0,0,0,0.03)",color:"#8B8578",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,fontSize:10}}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="14" rx="2" stroke="#8B8578" strokeWidth="1.5"/><circle cx="8.5" cy="10.5" r="2" stroke="#8B8578" strokeWidth="1.5"/><path d="M3 16l4-4 3 3 4-5 7 6" stroke="#8B8578" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                {T("addPhoto")}
              </button>
            )}
